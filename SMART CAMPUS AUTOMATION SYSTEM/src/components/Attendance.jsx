...
const Attendance = () => {
  ...
  const loadFaceApi = useCallback(async () => {
    ...
    try {
      toast({ title: "Loading", description: "Loading face recognition models..." });
  ...
  useEffect(() => {
    const load = async () => {
      // Demo dataset — 200 students
      const mockStudents = Array.from({ length: 200 }).map((_, i) => {
        const id = `211FA0${4000 + i}`;
        return {
          id: id,
      ...
      const mockFaculty = Array.from({ length: 20 }).map((_, i) => {
  ...
  }), [students, searchTerm, filter, selectedCourse, selectedYear, selectedBranch, selectedSection]);
